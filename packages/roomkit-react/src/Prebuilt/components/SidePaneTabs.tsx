...
import { BREAKOUT_ROOM_CODE, SIDE_PANE_OPTIONS } from "../common/constants";
...
export const SidePaneTabs = React.memo<{
  active: "Participants | Chat | Breakout Rooms";
  hideTab?: boolean;
}>(({ active = SIDE_PANE_OPTIONS.CHAT, hideTab = false }) => {
  const toggleChat = useSidepaneToggle(SIDE_PANE_OPTIONS.CHAT);
  const toggleParticipants = useSidepaneToggle(SIDE_PANE_OPTIONS.PARTICIPANTS);
  const toggleBreakoutRooms = useSidepaneToggle(
  ...
  const { enableBreakoutRooms, rolesToCreateBreakoutRooms } =
    useHMSPrebuiltContext();

  const localPeerID = useHMSStore(selectLocalPeerID);

  const peer = useHMSStore(selectPeerByID(localPeerID));
  const breakoutRoomCode = sessionStorage.getItem(BREAKOUT_ROOM_CODE);
  const canCreateBreakoutRooms =
    rolesToCreateBreakoutRooms?.includes(peer?.roleName || "") &&
    enableBreakoutRooms &&
    breakoutRoomCode === null;
  ...
  useEffect(() => {
    match({ activeTab, showChat, showParticipants })
      ...
      .with(
        {
          activeTab: SIDE_PANE_OPTIONS.BREAKOUT_ROOMS,
          showChat: false,
          showParticipants: false,
        },
        () => {
              ...
              <Flex css={{ w: "100%" }}>
                ...
                >
                  ...
                  {canCreateBreakoutRooms ? (
                    <Tabs.Trigger
                      value={SIDE_PANE_OPTIONS.BREAKOUT_ROOMS}
                ...
                {isOverlayChat && isChatOpen ? null : (
                  <IconButton
                    ...
                    onClick={(e) => {
                      ...
                      if (activeTab === SIDE_PANE_OPTIONS.CHAT) {
                        toggleChat();
                      } else if (activeTab === SIDE_PANE_OPTIONS.PARTICIPANTS) {
                        toggleParticipants();
                      } else {
                        toggleBreakoutRooms();
                      }
