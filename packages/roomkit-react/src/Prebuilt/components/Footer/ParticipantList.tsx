...
import { PaddedBox } from "../BreakoutRoom/BreakoutRoomsList";
...
export const ParticipantList = ({
  ...
  noBorderBottomRadius = false,
}: {
  offStageRoles: HMSRoleName[];
  onActive: (role: string) => void;
  setSelectedParticipants?: (participants: HMSPeer[]) => void;
  selectedParticipants?: HMSPeer[];
  noBorderBottomRadius?: boolean;
}) => {
  const [filter, setFilter] = useState<
    { search?: string; peerIds?: HMSPeerID[] } | undefined
  >();
  ...
  useEffect(() => {
    setFilter((filterValue) => {
      ...
      filterValue.peerIds = selectedParticipants?.map((p) => p.id);
      return { ...filterValue };
    });
  }, [selectedParticipants]);
    ...
    <Fragment>
      ...
      >
        <VirtualizedParticipants
          ...
          noBorderBottomRadius={noBorderBottomRadius}
...
export const Participant = ({
  ...
  setSelectedParticipants?: (peers: HMSPeer[]) => void;
  selectedParticipants?: HMSPeer[];
}) => {
  ...
  return (
    <Flex
      ...
      css={{
        ...
        borderRadius: "$1",
        borderBottomLeftRadius: noBorderBottomRadius ? 0 : "auto",
        borderBottomRightRadius: noBorderBottomRadius ? 0 : "auto",
        mt: "$8",
        flex: "1 1 0",
    ...
    >
      <Box>
        ...
        <HorizontalDivider css={{ mt: "$2" }} />
      </Box>
      <PaddedBox css={{ mt: "-$6" }}>
        {!filter?.search && participantsLength === 0 ? null : (
...
const ParticipantActions = React.memo(
  ({
    ...
    const { id: peerId, type: peerType, roleName: role = "" } = peer;
    ...
    const isBreakoutRoomPanelOpen = useIsSidepaneTypeOpen(
      SIDE_PANE_OPTIONS.BREAKOUT_ROOMS
    );
    return (
      <Flex
        align="center"
      ...
      >
        {isHandRaisedAccordion ? (
          ...
          <>
            ...
            {isBreakoutRoomPanelOpen ? (
              <Checkbox.Root
                id={`participant_${peerId}_checkbox`}
                data-testid={`participant_${peerId}_checkbox`}
                checked={
                  selectedParticipants?.find((p) => p.id === peerId) !==
                  undefined
                }
                onCheckedChange={(checked) => {
                  if (setSelectedParticipants) {
                    // @ts-ignore: No implicit Any
            ...
            {shouldShowMoreActions && !isLocal && !isBreakoutRoomPanelOpen ? (
    ...
    >
      ...
      <Input
        ...
        css={{
          ...
          pl: "$14",
          bg: "$surface_dim",
          border: inSidePane ? "1px solid $border_bright" : "",
        }}
        value={value}
        onKeyDown={(event) => {
