...
const useBroadcastMixer = () => {
  ...
  const { micMuted, setMicMuted } = useContext(UserSettingsContext);
  ...
  const removeDeviceFromRef = (deviceName) => {
    const filteredMixer = mixerRef.current.filter(
      (_device) => _device.name !== deviceName
    );
  ...
  const toggleMute = (deviceName) => {
    ...
    setMicMuted((prevState) => {
      ...
      toast({
        variant: "default",
        title: "Mic Status",
        description: `Mic is now ${prevState ? "unmuted" : "muted"}.`,
      });
      return !prevState;
    });
  };
