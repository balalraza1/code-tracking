...
export default function PreviewControlBar() {
  const { permissions, localAudioDeviceId } = useContext(LocalMediaContext);
  const { micMuted, cameraActive, role } = useContext(UserSettingsContext);
    ...
    <div className="flex items-center justify-center gap-4">
      <Tooltip content={micMuted ? "Unmute" : "Mute"}>
        <button
          ...
          } text-white rounded-full border-2 p-2 focus:outline-none`}
          onClick={() => {
            handleMicMute();
            console.log("preview control bar handle click");
          }}
