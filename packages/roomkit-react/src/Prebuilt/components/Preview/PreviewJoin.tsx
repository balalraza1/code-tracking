...
const PreviewJoin = ({
  ...
  const showNoMobileMessage =
    isMobile && localPeerRole?.includes(TEACHER_ROLE_NAME);
      ...
      >
        ...
        {showNoMobileMessage ? (
          <FixedNotificationContainer>
            Please join via laptop to start the session
          </FixedNotificationContainer>
        ) : null}
        <Flex
          direction="column"
          justify="center"
          css={{ w: "100%", maxWidth: "640px" }}
        >
          <Logo />
          <Text
            variant="h4"
        ...
        <Box
          css={{ w: "100%", maxWidth: `${Math.max(aspectRatio, 1) * 360}px` }}
        >
          <PreviewControls
